...
Para representar as rede de distribuição, foi escolhida a representação através de um grafo orientado, onde as arestas representam os elementos de conexão como condutores e chaves, e os vértices representam os pontos de carga ou sustentação da rede (para um modelo mais detalhado). Essa escolha foi feita por permitir uma implementação computacional mais eficaz e também uma formulação matemática que represente o problema. Como não é desejado que nenhuma resposta dessa metodologia provoque danos ao invés de soluções, por exemplo, uma manobra que sobrecarregue um condutor a ponto de rompê-lo. Para isso, a metodologia utilizará a seguinte função objetivo e restrições, as quais foram baseadas no trabalhando de \cite{Garcia2005}:
...
\begin{description}
    ...
    \item[$I'_{i}$] - Corrente no ramo fictício $i$;
    \item[$Z_i$] - Indica se o nó está sendo alimentado ($Z_i=1$) ou não ($Z_i=0$);
    \item[$IL_i$] - Capacidade de fluxo na aresta $i$;
    \item[$R_i$] - Impedância do nó $i$;
    \item[$L_i$] - Demanda do nó $i$;
    \item[$B_i$] - Conjunto de todas arestas com o nó $i$ como início;
...
Como função objetivo (\ref{eq:objetivo}), foi definido que deseja-se minimizar o número de consumidores não restabelecidos, que é o mesmo que tentar restabelecer o máximo de consumidores, uma vez que quanto mais consumidores restabelecidos, menos consumidores não restabelecidos. Em caso de empate (duas soluções restabeleçam o mesmo número de consumidores), como critério de desempate a escolha da manobra que tiver o menor carregamento máximo nos condutores. Deve-se notar o uso de arestas fictícias ($'$) para impedir que os valores de fluxos sejam negativos, nesse caso o valor do fluxo fica atribuídos para aresta fictícia. Para garantir o uso de apenas uma dessas arestas, foram inseridas as restrições \ref{eq:uso1aresta1} e \ref{eq:uso1aresta2}.

A restrição \ref{eq:radialidade} garante que as manobras de restabelecimento não podem inserir ciclos na rede de distribuição. Essa restrição garante que não existirá mais de uma chave fechada incidente em um nó da rede.
...
A restrição de carregamento dos condutores, restrições \ref{eq:capacidadeArestas1}, \ref{eq:capacidadeArestas2}, \ref{eq:capacidadeArestas3} e \ref{eq:capacidadeArestas4}, garante que não haverá sobrecarga nos mesmos. Para simplificar o modelo e não precisar descrever o modelo do fluxo de carga, os valores de carga nas variáveis $I_k$ são atribuídos por um algoritmo externo de cálculo de fluxo de potência por somatório de correntes \cite{bernardon2004desenvolvimento}. Esse algoritmofoi implementado dentro de uma ferramenta computacional será descrita mais a frente.

Como o tempo em que a rede irá permanecer no estado de contingência é de poucas horas, é pode ser permitido uma pequena carga acima do limite de operação dos condutores e um valor maior de carregamento nos equipamentos de proteção. Para simplificar o modelo, será considerado que os valores nas variáveis $IL_k$ já possuem esse valor incorporado. No decorrer do texto as referências a esses valores, que na metodologia são configuráveis, são referênciados como $F_{sobrecarga}$, que indica o quanto de sobrecarga será aceito nos condutores, $F_p$ indica o percentual de carregamento dos equipamentos de proteção.
...
Mesmo no estado de contingência é necessário manter a qualidade de energia disponibilizadas para os consumidores, assim as restrições \ref{eq:tensao} impõem os limites de tensão mínima e máxima. Essa restrição tem como principal objetivo evitar que manobras de transferência deixe os consumidores com uma tensão muito abaixo ou acima dos padrões estabelecidos.
